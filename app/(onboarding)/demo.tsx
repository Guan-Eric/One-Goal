...
import { View, Text, Pressable, TextInput, Platform, KeyboardAvoidingView, useColorScheme } from "react-native";
...
export default function DemoScreen() {
  ...
  return (
    ...
    ><View className="flex-1 bg-background">
        {/* Progress Dots */}
        <View className="px-8 pt-16 flex-row justify-center gap-2">
          ...
          {step === 1 && (
            <Animated.View entering={FadeIn.duration(400)}>
              ...
              <Pressable
                ...
                className={`py-5 rounded-full ${demoGoal.trim() ? 'bg-primary' : 'bg-surface-elevated'
                  }`}
              >
                <Text className={`text-center text-xl font-semibold ${demoGoal.trim() ? 'text-background' : 'text-text-muted'
                  }`}>
                  Set Goal
                </Text>
              </Pressable>
              ...
              >
                <View className="flex-row items-start justify-center">
                  <Animated.View
                    ...
                    className={`w-10 h-10 rounded-full mr-4 mt-2 items-center justify-center ${isCompleted ? 'bg-success' : 'border-2 border-text-secondary'
                      }`}
                  >
                    {isCompleted && (
                  ...
                  <View className="flex-1">
                    <Text
                      className={`text-3xl leading-tight text-center ${isCompleted ? 'text-text-secondary line-through' : 'text-text-primary'
                        }`}
                      style={{ fontFamily: "System" }}
                    >
                      {demoGoal}
            ...
            <Animated.View entering={FadeIn.duration(400)}>
              <View className="items-center mb-12">
                ...
                <View className="bg-surface-elevated rounded-2xl p-6 w-full">
                  ...
                  <BenefitItem icon="history" text="Full goal history" iconColor={iconColor} />
                  <BenefitItem icon="cloud-sync" text="Sync across devices" iconColor={iconColor} />
                  <BenefitItem icon="chart-line" text="Advanced statistics" iconColor={iconColor} />
                  <BenefitItem icon="palette" text="Beautiful themes" iconColor={iconColor} />
                </View>
              </View>
...
function BenefitItem({ icon, text, iconColor }: { icon: string; text: string; iconColor: string }) {
